<template>
  ...
  >
   ...
   <div class="absolute inset-0 bg-black/40 backdrop-blur-sm"></div>
    ...
    <!-- Glass card -->
    <div
      class="flex w-[800px] h-[500px] rounded-2xl overflow-hidden shadow-2xl bg-white/10 backdrop-blur-md"
    >
      <!-- Left side (image + text) -->
      <div
        class="w-1/2 bg-cover bg-center flex flex-col justify-end text-white p-8"
        style="background-image: url('https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=900&q=80')"
      >
      ...
      <!-- Right side (form) -->
      <div class="w-1/2 bg-black/40 flex flex-col justify-center p-10 text-white">
        ...
        <p class="text-md mt-6 text-gray-300 ">
          Have an account? <router-link to="/login" class="hover:underline hover:bg-white/15 px-1 py-1 rounded">Log in</router-link>
        </p>
      </div>
    </div>
  </div>
  ...
  methods: {
    async submit() {
      ...
      try {
        ...
        // login right after registration because we dont have a token implementetion in the register endpoint
