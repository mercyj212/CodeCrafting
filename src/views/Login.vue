<template>
  <div
    class="relative min-h-screen flex items-center justify-center bg-[#d6d4d2] bg-cover bg-center px-4 py-8"
  ...
  >
    ...
    <!-- Glass card -->
    <div
      class="relative w-full max-w-md h-[500px] rounded-2xl p-8 text-white shadow-lg bg-black/40 backdrop-blur-md"
    >
      <h2 class="text-xl md:text-2xl font-bold mb-6 mt-4 flex items-center justify-center">
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        >
          <img src="../assets/user.png" alt="User Icon" class="w-10 h-10" />
        </div>
        ...
        <div>
          <input
            ...
            class="w-full bg-transparent border-b border-white/50 focus:outline-none py-2 placeholder-white/70 text-lg"
          />
        </div>
        ...
        <div class="flex items-center justify-between text-sm flex-wrap gap-2">
          ...
          <label class="flex items-center space-x-2">
            <input type="checkbox" v-model="remember" class="w-3 h-3 rounded" />
            <span>Remember me</span>
          </label>

          <router-link class="text-sm text-gray-200 hover:underline">
            Forgotten password?
          </router-link>
        </div>
        ...
        <div class="space-y-6">
          ...
          <p class="text-center text-m mt-4 text-gray-200">
            ...
            <router-link
              to="/register"
              class="hover:bg-white/15 hover:underline px-2 py-1 rounded"
              >Sign Up</router-link
            >
          </p>
        </div>
  ...
  methods: {
    async submit() {
      ...
      try {
        ...
        // Save email if remember is checked, else remove
        if (this.remember) {
